...
<div class="wrapper">
    ...
    <div class="rightbar">
            ...
            <div style="float:right; padding-top:20px;">
            	<a href="<?=base_url().'admin/countries/add'?>" class="top_link"><img src="<?=base_url()?>btPublic/admin/images/icons-big/add-large.png" width="24" height="24" />Add New Country</a>
            </div>
            <div class="search_DIV">
            	<?php
				$data['controller__Name'] = 'countries';								
			...
			if((bool)$QUERY > 0){
			?>            
            	<form id="FRM_ACTIVE_ALL" action="<?=base_url().'admin/countries/performAction/'.$page?>" method="post">
            	<div class="grid-bg">
            	
                <ul>
                	<li><a href="<?=base_url().'admin/countries/add'?>">Add New</a></li>
                ...
                <div class="grid-data2"><a href="<?=base_url().'admin/countries/Sort/ID'?>">#</a></div>
                <div class="grid-data" style="width:200px; color:#000;"><a href="<?=base_url().'admin/countries/Sort/name'?>">Country Name</a></div>                
                <div class="grid-data" style="color:#000; width:340px;"><a href="<?=base_url().'admin/countries/Sort/date'?>">Date Added</a></div>
                ...
                ?>
                    <div class="grid-head" style="background:<?=$bgColor?>;">                    	
                        ...
                        <div class="grid-data" style="width:200px;"><a href="<?=base_url().'admin/countries/update/'.$row->ID.'/'.$page?>" title="Edit / Update..." style="font-weight:normal; color:#999999;"><?=$row->name?></a></div>                        
                        <div class="grid-data" style="width:340px;"><?=displayDate($row->date)?></div>
                        <div class="grid-data" style="width:120px;"><a href="<?=base_url().'admin/countries/changeStatus/'.$row->ID.'/'.$row->status.'/'.$page?>" style="color:#999999; font-weight:normal;"><?=showStatus($row->status)?></a></div>
                        <div class="grid-data">
                            <div style="float:left; padding-right:7px;"><a href="<?=base_url().'admin/countries/update/'.$row->ID.'/'.$page?>" title="Edit / Update..."><img src="<?=base_url()?>btPublic/admin/images/edit.png" style="padding-top:3px;" /></a></div>
                            <div style="float:left;"> | </div>
                            <div style="float:left; padding-left:7px;"><a href="javascript: void(0);" title="Remove..." onclick="deleteEntry('<?=base_url().'admin/countries/delete/'.$row->ID.'/'.$page?>')"><img src="<?=base_url()?>btPublic/admin/images/delete.png" style="margin-top:3px;" /></a></div>
                        </div>
                    </div>
				<?php
				}
            ...
            <?php
				...
				if($page > 0){
					redirect(base_url().'admin/countries/'.($page-$per_page), 'refresh');
				}
				if((bool)($this->session->userdata($sessionSearch)) == TRUE){										
					$this->load->view('admin/searchResults_view', $data);
				}
				else{
					echo '<div class="info_MSG">No country available for this section.<br/>Click <a href="'.base_url().'admin/countries/add">HERE</a> to add new country.</div>';
				}
			}
			?>
        </div>
