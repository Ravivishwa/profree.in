...
class Payment extends CI_Controller {
	...
	private $Table = 'tbl_payment_options';
	...
	public function index()
	{
		...
		$this->load->view('admin/payment/paymentView', $data);
	}
	...
	public function add(){		
		...
		$data['Message'] = '';	
		$data['heading'] = 'Payment Plan: Add Payment Plan';
		$data['pageName'] = 'payment';				
		$isValid = false;
		$data['site_currency'] = $this->general_model->single_value("ID = '1'", 'currency', 'tbl_settings');
		$data['title'] = '';
		...
		$data['properties'] = 0;
		$data['time_period'] = 0;
		if((bool)($this->input->post('btnAddpayment')) == TRUE){
			...
			if($isValid){
				...
				$this->load->view('admin/payment/addpaymentView', $data);
			}
		}
		else{
			$this->load->view('admin/payment/addpaymentView', $data);
	...
	public function update($ID = NULL, $page = NULL){		
		$data['pageTitle'] = $this->project_model->projectName().' :: payment : Update Payment Plan';
		$data['Message'] = '';	
		$data['heading'] = 'Payment Plan: Update Payment Plan';
		$data['pageName'] = 'payment';
		$data['page'] = $page;
		$data['ID'] = $ID;
		$isValid = false;
		...
		if((bool)($this->input->post('btnUpdatepayment')) == TRUE){
			...
			if($this->form_validation->run() === TRUE){																
				...
				if($isValid){
					...
					$this->load->view('admin/payment/editpaymentView', $data);
				}
			}
			else{
				$this->load->view('admin/payment/editpaymentView', $data);
